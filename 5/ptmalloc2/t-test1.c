...
malloc_test(struct thread_st *st)
{
	...
	mstate ar_ptr = _int_get_arena(0);
	struct malloc_arena_info mai;
	struct malloc_global_info mgi;
	_int_get_arena_info(ar_ptr, &mai);
	_int_get_global_info(&mgi);

	/* printf("nfastblocks     = %lu\nnbinblocks     = %lu\nfastavail = %lu\nbinavail = %lu\ntop_size  = %lu\nsystem_mem = %lu/%lu\nn_mmaps = %lu/%lu\nmmapped_mem = %lu/%lu\nmax_total_mem = %lu\nstat_n_heaps = %lu\n",
	 (unsigned long)mai.nfastblocks,
	 (unsigned long)mai.nbinblocks,
	 (unsigned long)mai.fastavail,
	 (unsigned long)mai.binavail,
	 (unsigned long)mai.top_size,
	 (unsigned long)mai.system_mem,
	 (unsigned long)mai.max_system_mem,
	 (unsigned long)mgi.n_mmaps,
	 (unsigned long)mgi.max_n_mmaps,
	 (unsigned long)mgi.mmapped_mem,
	 (unsigned long)mgi.max_mmapped_mem,
	 (unsigned long)mgi.max_total_mem,
	 (unsigned long)mgi.stat_n_heaps);*/

	mbinptr ub;
  mchunkptr up;
	int ui;

	printf("\n===FAST BINS===\nNumber of fast bins = %d\n", NFASTBINS);
	for (ui = 0; ui < NFASTBINS; ++ui) {
		size_t fastavail = 0;
		int nfastblocks = 0;
    for (up = ar_ptr->fastbins[ui]; up != 0; up = up->fd) {
      ++nfastblocks;
      fastavail += chunksize(up);
    }
		printf("Fast Bin %d\nnblocks = %lu\navail = ",
		 ui+1,
		 (unsigned long)nfastblocks);
		for (up = ar_ptr->fastbins[ui]; up != 0; up = up->fd) {
      fastavail = chunksize(up);
      printf("%d ", fastavail);
    }
	...
	for(ui = 1; ui<NBINS; ui++) {
		...
		int nubinblocks = 0;
		ub = bin_at(ar_ptr, ui);
		for (up = last(ub); up != ub; up = up->bk) {
      ++nubinblocks;
    }
		printf("Bin %d\nnblocks = %lu\navail = ",
		 ui,
		 (unsigned long)nubinblocks);
		for (up = last(ub); up != ub; up = up->bk) {
      ubinavail = chunksize(up);
      printf("%d ", ubinavail);
    }
		printf("\n");
	}
